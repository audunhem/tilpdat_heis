...
void fsm_arrive_at_floor(struct ElevatorData* elevator){
  ...
  elev_set_floor_indicator(elevator->current_floor);
	...
	if (decision_check_if_should_stop(elevator)) {
...
void fsm_order_button_pressed(struct ButtonPress order, struct ElevatorData* elevator){
    ...
    elev_set_motor_direction(decision_next_motor_direction(elevator));
    elevator->stopped_between_floors = false;

    if (decision_next_motor_direction(elevator) == DIRN_STOP) {
...
void fsm_stop_button_pressed(struct ElevatorData* elevator){
  ...
  elev_set_motor_direction(DIRN_STOP);
  timer_stop();
  decision_reset_all_orders(elevator);
  fsm_set_lights(elevator->orders);
